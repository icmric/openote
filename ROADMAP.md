...
8. ~~CI~~ — **added 2026-07-27** (`.github/workflows/ci.yml`): analyze + build + test and `cargo test`/`clippy` on Windows, macOS and Linux, plus a `cargo-deny` job guarding the licensing invariant. **Never executed** — the workflow has not run on a remote yet, so the cross-platform claim remains unverified until it does. The macOS and Linux jobs are the first real test of PLAT-1.
