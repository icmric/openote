...
> **Status: v0.7.1 released** · **1,071 Dart + 53 Rust tests green, analyzer clean** (verified 2026-08-11) · Last updated 2026-08-11
>
...
>
> **What to do next**, ranked: [the standing backlog §1](docs/planning/v0.4-and-beyond.md#1-what-to-do-next). The short version — **use the macOS and Linux builds** (three platforms ship, two have never been run by a human), **split `AppState`** (5,901 lines, larger than before its last split), then **demote the container** (its blocker was retired by the git-join work) and **collect blobs** (nothing has ever deleted one). The highest-priority *unstarted plan* is [making folder sync bomb-proof](docs/planning/v0.12-folder-sync-hardening.md), which its own header marks as coming before any more git work.
...
**The ranked list lives in [the standing backlog §1](docs/planning/v0.4-and-beyond.md#1-what-to-do-next)** and is not duplicated here. In short: use the macOS and Linux builds, split `AppState`, demote the container, collect blobs.
