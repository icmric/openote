...
impl<R: Read + Seek> Cabinet<R> {
    ...
    /// Decompress an entire folder's uncompressed stream in ONE pass, up to
    /// `limit` bytes. (Openote patch.) Reading files individually via
    /// [`read_file`](Self::read_file) re-decompresses the folder prefix for
    /// every file — quadratic over a many-file folder; callers can instead
    /// slice this buffer at each entry's `uncompressed_offset()`.
    pub fn read_folder_data(
        &mut self,
        index: usize,
        limit: u64,
    ) -> io::Result<Vec<u8>> {
