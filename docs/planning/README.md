...
|---|---|---|
| [v0.24 — the road to 1.0](v0.24-road-to-1.0.md) | **review done; the translation foundation and the onboarding rebuild shipped 2026-09-02** | A whole-project review with every figure measured against the tree, a definition of what 1.0 should actually promise (the format is frozen, the three platforms it claims have been *used*, nothing it does to your notes is reversible only by hand), and a three-revision order to get there. Names the one real blocker — **macOS and Linux have never been opened by a human** — and the one performance property worth fixing first: every keystroke rebuilds the whole shell, ~39 ms of it chrome that cannot have changed. Carries the §Languages contract every `.arb` and `l10n.yaml` points at. |
...
| [v0.21 — outdo OneNote at maths](v0.21-outdo-onenote-maths.md) | **planned; ranked from the 2026-08-20 four-analyst review** | The competitive half of the full maths review: aligned derivations (quick-win), graphing and solve-for-x (undercut the M365 paywall — graphing needs a vision amendment), units, chemistry, equation numbering, spoken maths. Ink-to-math deferred with reasons recorded. |
| [v0.22 - graphs](v0.22-graphs.md) | **designed; needs one vision call before step 4** | The full graph design, every number measured: compile-once closures (33x faster than the only alternative, which costs 87% of a frame), what a student presses, what the block looks like, storage, and an honest 3D verdict (0.6ms/frame at 60x60 in pure Dart - later, not skip). Carries four calculator bugs that would each become a visibly wrong curve. |
...
| [v0.17 — the storage cluster](v0.17-storage-cluster.md) | **shipped** (Steps 1–9 including 8a/8b, 2026-08-15/16) | Tasks #39/#54/#55, re-costed on the owner's real notebooks after Phase 1 replaced the world they were written for. #54 closed outright on measurement (segments, gzip, snapshots and compaction all cut, with reproduced data loss on a real log); the demotion kept and re-argued as a *correctness* fix — a live WAL SQLite database was being replicated by Google Drive. Delivered the four product decisions of 2026-08-15 (simplified version history in place of the 30-deep snapshot table, register the notebook **folder** rather than the `.onote` file, **the log is the open format**, ship macOS without a human pass), the container demotion (opt-in per notebook, reversible via `undemote`), and — across several follow-up fixes the same week — the end of pictures living twice on disk. The doc's own top-of-file status line still says "Steps 1 and 2 shipped"; trust this row and the commit history over it. |
| [v0.12 — make folder sync bomb-proof](v0.12-folder-sync-hardening.md) | **shipped, 2026-08-15** | All three reported faults fixed: the watcher missed a cloud client's temp-file-rename (fixed, plus a 5 s poll as a floor regardless of cause), the manual pull's 10–20 s freeze (paced into per-frame chunks), and deleted notebooks leaving files behind / setup offering phantom notebooks (purge now refuses to delete another device's shared logs; onboarding validates a `.onote`/`.onotebook` pair instead of trusting a filename). Both tests §3 asked for exist (`folder_watch_test.dart`, `cloud_sync_test.dart`) and pass. No longer the highest-priority unstarted plan — nothing here is unstarted. |
