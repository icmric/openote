...
|---|---|---|
| [v0.18 — maths without the syntax](v0.18-visual-maths.md) | **planned, nothing built** | The OneNote-style visual equation editor: build-as-you-type in place with boxes to fill, the full student symbol inventory as one data table driving palette/search/tooltips/tests, a searchable maths bar docked at the equation, and equations editable inside the sentence. Carries a measured grammar footgun (`$5 and $10` in one sentence becomes an equation today) and six product decisions awaiting the owner (§11). |
