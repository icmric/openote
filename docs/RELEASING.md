...
|---|---|---|
| Windows | `openote-X.Y.Z-windows-x64-setup.exe` | Runs it. Wizard, Start-menu entry, uninstaller, and the `Open this notebook` file inside a notebook folder starts opening in Openote. **No administrator password** — it installs per-user into `%LOCALAPPDATA%`. From v0.17 it also *removes* the old `.onote` association, on upgrade as well as on uninstall — see the deletion entries at the top of `packaging/windows/openote.iss` and why they cannot be an omission. |
| Windows | `openote-X.Y.Z-windows-x64.zip` | Unzips it, runs `openote.exe`. Kept for locked-down machines, USB sticks, and people who want to look inside first. **No file association** — that lives in the installer's registry entries, so double-clicking a notebook does nothing here. It also means the zip never retires an old `.onote` association: somebody who moves from the installer to the zip keeps whatever their last installer left. |
| Linux | `openote-X.Y.Z-linux-amd64.deb` | Double-click on Ubuntu/Debian/Mint. Installs to `/opt`, adds a menu entry and the notebook association. **Whether a double-click on a `.onotebook` folder opens Openote rather than the folder is per-file-manager and unproven** — check GNOME Files and Dolphin before claiming it in release notes. The `Open this notebook` file inside works either way. |
...
| macOS | `openote-X.Y.Z-macos-universal.dmg` | Opens, drags to Applications. Universal — one file for Intel and Apple Silicon. **No notebook association yet, and v0.17 did not add one.** macOS delivers files through `application(_:open:)` rather than as an argument, and `Runner/AppDelegate.swift` does not override it — so declaring the document type in `Runner/Info.plist` would make Finder launch Openote on whatever notebook was last open, which is the bug commit `39e5d33` fixed everywhere else. `LSTypeIsPackage` on `.onotebook` would additionally hide the folder's contents in Finder, so it must not ship before the delivery half does. Both halves belong together, in the macOS packaging item. |
